 ...
 */
#define NEAR (200) // valore da testare
...
int extract(void * indexe, ulong from, ulong to, uchar **dest, 
	...
	ulong row = 0; /* numero di riga corrispondente all'ultima position */
	...
	/* Legge sequenzalimente le posizioni marcate. E' garantita la presenza 
	   di tutte le posizioni divisibili per index->skip */
	
	scarto = index->skip - (to%index->skip);
	//if (scarto == index->skip) scarto = 0;
	
	ulong to_new = to + scarto-1;
	if (to_new >= real_text_size-index->skip-1) { // vicina alla fine del testo la riga e' la 0
			row = index->bwt_eof_pos;
			scarto = real_text_size - to-1;
	}
	/* Inizia a leggere le posizioni finche non trova to_new */
	fm_init_bit_reader(index->start_prologue_occ);
	for(i=0; i<index->num_marked_rows; i++) { 
		pos_text = fm_bit_read(index->log2textsize);

		if((pos_text >= to_new) &&(pos_text < to_new+NEAR)){
							scarto += pos_text-to_new;
							row = i + index->occcharinf;
				  			break;
		}
	}		
