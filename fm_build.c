...
int fm_build_config (fm_index *index, suint tc, double freq, 
	...
	if (freq >= 0.5) { index->skip = 1; return FM_OK;}
	if(freq == 0) { index->skip = 0; return FM_OK;}
	index->skip = (suint) 1/freq;	/* 1/Mark_freq 2% text size */
	return FM_OK;
}
...
int fm_build(fm_index *index, uchar *text, ulong length) {
	...
	#endif
	if(index->text_size < SMALLSMALLFILESIZE) index->skip = 0;
